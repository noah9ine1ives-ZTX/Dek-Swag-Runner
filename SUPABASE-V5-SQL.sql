...
-- SWAG NIGHT RUNNER v5.0 CLEAN FOUNDATION OVERHAUL
-- New clean character/item foundation. Safe to run repeatedly.
alter table public.profiles
add column if not exists body_type text not null default 'male';

alter table public.profiles
add column if not exists effect_intensity text not null default 'medium';

alter table public.profiles
add column if not exists reduce_vfx boolean not null default false;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'profiles_body_type_chk') then
    alter table public.profiles
    add constraint profiles_body_type_chk check (body_type in ('male','female')) not valid;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'profiles_effect_intensity_chk') then
    alter table public.profiles
    add constraint profiles_effect_intensity_chk check (effect_intensity in ('low','medium','high')) not valid;
  end if;
end $$;
...
alter table public.player_loadouts
add column if not exists trail_effect text not null default 'none';

alter table public.player_inventory
add column if not exists accessories text[] not null default array['none'];

alter table public.player_inventory
add column if not exists trail_effects text[] not null default array['none'];

alter table public.player_loadouts
alter column hair set default 'basic-short';

alter table public.player_loadouts
alter column top set default 'plain-tee';

alter table public.player_loadouts
alter column pants set default 'basic-pants';

alter table public.player_loadouts
alter column shoes set default 'simple-sneakers';

alter table public.player_loadouts
alter column trail_effect set default 'none';
