...
function showDevToast(type,message){
 ...
 async function refreshDevTarget(manual=false){
  if(!supabase||!user||!isDev||!devTarget?.id)return;
  if(manual)showDevToast("loading","Processing: refreshing target...");
  try{
   const {data,error}=await supabase.from("profiles").select("id,username,friend_code,role,account_status,suspended_until,coins,best_score,best_combo,best_style,bio,avatar_url,last_seen,created_at,updated_at").eq("id",devTarget.id).maybeSingle();
   if(error)throw error;
   if(!data)throw new Error("target not found");
   setDevTarget(data);setDevTargetCoins(String(data.coins||0));setDevTargetRole(data.role||"player");
   if(manual)showDevToast("success","Success: target refreshed");
  }catch(e){
   if(manual)showDevToast("error",`Failed to refresh target: ${e.message}`);
   else throw e;
  }
 }
 ...
 const routeMainMenu=(target)=>{
  if(locked)return lockToast();
  const authOnly=["play","friends","profile","settings","dev"];
  if(authOnly.includes(target)&&!user){
   setActive("settings");
   setScreen("settings");
   setToast("กรุณา Login ก่อนใช้งานเมนูนี้");
   return;
  }
  if(target==="dev"&&!isDev){
   setToast("Dev only");
...
async function devSyncWallet(nextCoins){
 ...
 async function devAudit(action,targetId,details={}){
  ...
  }catch(e){console.error("audit failed",e);throw e}
 }
 async function devLookupPlayer(){
  ...
  try{
   ...
   showDevToast("success",`Success: target refreshed`);
 ...
 async function devSetTargetCoins(){
  ...
  try{
   ...
   const auditErr=await safeAudit("set_coins",devTarget.id,{reason:devReason,old:devTarget.coins,new:coins,target_username:devTarget.username});
   await refreshDevTarget(false);
   showDevToast("success","Success: coins updated");
   if(auditErr)showDevToast("warning",`Action completed, but audit log failed: ${auditErr.message}`);
  }catch(e){console.error(e);showDevToast("error",explainDevError(e,"Failed: Supabase error"))}
  finally{setDevBusy(false)}
 }
 async function devAddTargetCoins(){
  ...
  setDevBusy(true);
  try{
   showDevToast("loading","Processing: adding coins...");
   await supabase.from("profiles").update({coins,updated_at:new Date().toISOString()}).eq("id",devTarget.id);
   const auditErr=await safeAudit("add_coins",devTarget.id,{reason:devReason,amount:add,old:devTarget.coins,new:coins,target_username:devTarget.username});
   await refreshDevTarget(false);
   showDevToast("success","Success: coins added");
   if(auditErr)showDevToast("warning",`Action completed, but audit log failed: ${auditErr.message}`);
  }catch(e){console.error(e);showDevToast("error",explainDevError(e,"Failed: Supabase error"))}
 ...
 if(!requireDevSafetyCode())return;
  ...
  showDevToast("success","Success: leaderboard score removed");
...
function requireDevSafetyCode(){
 ...
 async function devClearGlobalLeaderboard(){
  ...
  try{
   ...
   const auditErr=await safeAudit("clear_leaderboard",null,{reason:devReason,deleted_count:count??null});
   await loadGlobalLeaderboard();await loadFriendsLeaderboard();await devRefreshLogs();
   showDevToast("success","Success: global leaderboard cleared");
   if(auditErr)showDevToast("warning",`Action completed, but audit log failed: ${auditErr.message}`);
  }catch(e){console.error(e);showDevToast("error",`Failed to clear leaderboard: ${e.message}`)}
  finally{setDevBusy(false)}
 }
...
g.trails.forEach(t=>{t.life--;t.x+=(t.vx||(-g.speed*.75*boost));t.y+=(t.vy||0);t.vy=(t.vy||0)+.03});g.trails=g.trails.filter(t=>t.life>0).slice(-42);g.particles.forEach(pt=>{pt.x+=pt.vx;pt.y+=pt.vy;pt.vy+=.25;pt.life--});g.particles=g.particles.filter(pt=>pt.life>0);g.texts.forEach(t=>{t.y-=.75;t.life--});g.texts=g.texts.filter(t=>t.life>0);g.build.forEach(b=>{b.x-=g.speed*.28*boost;if(b.x+b.w<0){b.x=W+Math.random()*120;b.w=70+Math.random()*45;b.h=90+Math.random()*190;b.win=Math.floor(4+Math.random()*7)}});g.stars.forEach(s=>{s.x-=g.speed*.08*boost;if(s.x<0)s.x=W});g.rain.forEach(r=>{r.x-=g.speed*.35*boost;r.y+=r.spd*boost;if(r.y>H+30||r.x<-30){r.x=Math.random()*W+40;r.y=-30}});const now=performance.now();if(now-ui.current>100||g.lives<=0){ui.current=now;setScore(g.score);setCoins(g.coins);setLives(g.lives);setCombo(Math.floor(g.combo));setMaxCombo(g.maxCombo);setStyleScore(g.style)}}
 ...
 return <div className="app play-screen"><div className="shell play-shell"><div className="play-topbar"><div>{status==="gameover"?<button className="btn ghost"onClick={goLanding}>← กลับหน้าแรก</button>:<div className="muted">กำลังวิ่งอยู่</div>}<h1 className="play-title">Swag Night Runner</h1><div className="muted">Player: <b>{playerName}</b> • {toast}</div></div><div className="play-actions"><Button onClick={reset}>{status==="playing"?"↻ เริ่มใหม่":user?"▶ Start":"🔐 Login to Play"}</Button><Button variant="outline"onClick={goLanding}>Menu</Button></div></div><div className="stats play-stats">{[["Score",score],["Coins",`🪙 ${coins}`],["Lives",`❤️ ${lives}`],["Best",bestScore],["Combo",`x${combo}`],["Max",`x${maxCombo}`],["Style",styleScore],["Wallet",walletCoins]].map(([a,b],idx)=><div className={`stat ${idx>2?"play-extra-stat":""}`}key={a}><small>{a}</small><b>{b}</b></div>)}</div><div className="canvaswrap play-canvaswrap"><canvas ref={canvasRef}width={W}height={H}onTouchStart={e=>{e.preventDefault();const t=e.changedTouches[0],r=e.currentTarget.getBoundingClientRect();if(t.clientX-r.left<r.width*.42)pressDuck(true);else pressJump()}}onTouchMove={e=>e.preventDefault()}onTouchEnd={e=>{e.preventDefault();pressDuck(false)}}/><div className="mobilehint"><span>แตะซ้ายค้าง = หมอบ</span><span>แตะขวา = กระโดด</span></div></div>{status==="gameover"&&<div className="result"><div className="card pad"><div className="muted">RESULT</div><div className="grade">Rank {grade(finalScore,maxCombo,styleScore)}</div><p>Score <b>{finalScore}</b> • Max Combo <b>x{maxCombo}</b></p><p>Style <b>{styleScore}</b> • Earned <b>+{coins}</b></p><p className="muted">Wallet รวมตอนนี้ <b>{walletCoins}</b></p><div className="row"><Button onClick={reset}>เล่นอีกครั้ง</Button><Button variant="outline"onClick={goLanding}>กลับหน้าแรก</Button></div></div></div>}<div className="grid4 play-controls"style={{marginTop:12}}><button className="btn dark"onTouchStart={e=>{e.preventDefault();pressDuck(true)}}onTouchEnd={e=>{e.preventDefault();pressDuck(false)}}onMouseDown={()=>pressDuck(true)}onMouseUp={()=>pressDuck(false)}>↓ หมอบ</button><button className="btn"onTouchStart={e=>{e.preventDefault();pressJump()}}onMouseDown={pressJump}>↑ กระโดด</button><button className="btn dark"disabled={locked}onClick={cycleQuality}>⚙️ Quality: {quality.toUpperCase()}</button></div></div></div>
}
