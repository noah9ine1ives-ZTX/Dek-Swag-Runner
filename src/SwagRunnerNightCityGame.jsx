...
export default function SwagRunnerNightCityGame(){
 ...
 function fresh(){const low=quality==="low"||(quality==="auto"&&innerWidth<768);return{visual:low?"low":"high",speed:5,distance:0,score:0,bonus:0,anim:0,coins:0,lives:3,combo:1,timer:0,maxCombo:1,style:0,dodges:0,gameOver:false,inv:0,shield:0,magnet:0,boost:0,heat:0,shake:0,player:{x:120,y:GROUND-76,w:46,h:76,vy:0,on:true,duck:false,jumpCount:0,maxJumps:2,jumpPressed:false,justDoubleJumped:false},obs:[],coinItems:[],power:[],particles:[],texts:[],trails:[],nextObs:90,nextCoin:45,nextPower:260,build:Array.from({length:12},(_,i)=>({x:i*95,w:70+Math.random()*45,h:90+Math.random()*190,win:Math.floor(4+Math.random()*7)})),stars:Array.from({length:low?40:70},()=>({x:Math.random()*W,y:Math.random()*240,r:Math.random()*1.6+.4})),rain:Array.from({length:low?42:95},()=>({x:Math.random()*W,y:Math.random()*H,len:8+Math.random()*18,spd:8+Math.random()*7,a:.12+Math.random()*.22}))}}
...
function previewBodyType(type){if(locked)return lockToast();const nl=normLoadout({...previewLoadout,bodyType:type});setPreviewLoadout(nl);setPreviewItem({cat:"bodyType",id:type,name:type==="female"?"Female Runner":"Male Runner",price:0});setToast(`Preview ${type==="female"?"Female Runner":"Male Runner"}`)}
 ...
 function update(){const g=game.current,rt=state.current;if(rt.screen!=="game"||rt.status!=="playing")return;const p=g.player,boost=g.boost>0?1.75:1,jump=!!(keys.current.Space||keys.current.ArrowUp||keys.current.KeyW),jumpStart=jump&&!p.jumpPressed;p.jumpPressed=jump;p.duck=!!(keys.current.ArrowDown||keys.current.KeyS);
