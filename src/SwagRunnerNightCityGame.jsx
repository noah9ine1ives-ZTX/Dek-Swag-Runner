...
function previewBodyType(type){if(locked)return lockToast();const nl=normLoadout({...previewLoadout,bodyType:type});setPreviewLoadout(nl);setPreviewItem({cat:"bodyType",id:type,name:type==="female"?"Female Runner":"Male Runner",price:0});setToast(`Preview ${type==="female"?"Female Runner":"Male Runner"}`)}
 ...
 useEffect(()=>{
  ...
  const ps=parts(previewLoadout),ef=EFFECTS.find(e=>e.id===previewSkin)||EFFECTS[0],isFemale=ps.bodyType==="female";
  ctx.save();ctx.translate(82,40);ctx.scale(isFemale?1.22:1.25,isFemale?1.24:1.25);ctx.shadowColor=ef.glow;ctx.shadowBlur=15;
  ...
  if(ps.top.style==="plainTee"){ctx.fillStyle=ps.top.main;rr(isFemale?10:8,hy+3,isFemale?28:32,26,9);ctx.fill();ctx.fillStyle=ps.top.accent;ctx.fillRect(isFemale?15:13,hy+10,isFemale?18:22,3)}else if(ps.top.style==="hoodie"){const gr=ctx.createLinearGradient(0,hy,48,hy+34);gr.addColorStop(0,ps.top.accent);gr.addColorStop(.18,ps.top.main);gr.addColorStop(1,"#09090b");ctx.fillStyle=gr;rr(isFemale?7:5,hy,isFemale?34:38,30,13);ctx.fill();ctx.fillStyle=ps.top.detail;rr(isFemale?11:9,hy+17,isFemale?26:30,13,7);ctx.fill()}
  ...
  ctx.fillStyle="#0f172a";ctx.beginPath();ctx.arc(19,15,1.7,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(29,15,1.7,0,Math.PI*2);ctx.fill();if(isFemale){ctx.strokeStyle="rgba(15,23,42,.55)";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(16,13);ctx.lineTo(13,12);ctx.moveTo(32,13);ctx.lineTo(35,12);ctx.stroke()}ctx.fillStyle="rgba(244,114,182,.38)";ctx.beginPath();ctx.arc(16,19,3,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(32,19,3,0,Math.PI*2);ctx.fill();ctx.restore();
 ...
 const renderMiniPreview=()=> <div className="avatar-preview"><canvas ref={previewCanvasRef}/><div className="style-preview-info"><b>{previewItem?.name||"Preview Outfit"}</b><span>{parts(previewLoadout).bodyType==="female"?"Female Runner":"Male Runner"}</span><small>{parts(previewLoadout).hair.name} • {parts(previewLoadout).top.name}</small><small>{parts(previewLoadout).pants.name} • {parts(previewLoadout).shoes.name}</small><small>{parts(previewLoadout).accessory.name} • {TRAIL_EFFECTS.find(e=>e.id===previewSkin)?.name||"No Trail"}</small></div></div>;
