...
const TRAIL_EFFECTS=[
 {id:"none",name:"No Trail",price:0,rarity:"starter",accent:"#64748b",glow:"rgba(100,116,139,.10)",trail:"rgba(148,163,184,.16)",type:"none",desc:"เริ่มต้นแบบไม่มีเอฟเฟกต์"},
 {id:"ember-step",name:"Ember Step",price:450,rarity:"rare",accent:"#f97316",glow:"rgba(249,115,22,.30)",trail:"rgba(251,146,60,.46)",type:"ember",desc:"ไฟลากจากเท้าแบบ runner"},
 {id:"neon-blue-step",name:"Neon Blue Step",price:600,rarity:"rare",accent:"#38bdf8",glow:"rgba(56,189,248,.30)",trail:"rgba(56,189,248,.42)",type:"neon",desc:"แสงฟ้านีออนติดเท้า"},
 {id:"lightning-step",name:"Lightning Step",price:900,rarity:"epic",accent:"#fde047",glow:"rgba(250,204,21,.36)",trail:"rgba(253,224,71,.50)",type:"lightning",desc:"สายฟ้าสั้น ๆ ตอนวิ่ง"},
 {id:"smoke-drift",name:"Smoke Drift",price:700,rarity:"rare",accent:"#94a3b8",glow:"rgba(148,163,184,.20)",trail:"rgba(203,213,225,.28)",type:"smoke",desc:"ควันนุ่ม ๆ เล่นนานไม่ล้า"},
 {id:"holo-dust",name:"Holo Dust",price:1100,rarity:"epic",accent:"#a78bfa",glow:"rgba(167,139,250,.28)",trail:"rgba(216,180,254,.34)",type:"holo",desc:"ละออง hologram เบา ๆ"}
];
...
export default function SwagRunnerNightCityGame(){
 ...
 const state=useRef({screen:"landing",status:"ready",playerName:"SWAGPLAYER",skin:"none",quality:"auto",loadout:DEFAULT_LOADOUT,saved:null});
 ...
 async function devResetTargetStyle Shop(){
  if(!supabase||!user||!isDev||!devTarget)return setToast("Dev only");
 ...
 function drawAvatar(ctx,p,lo,sk){const ef=TRAIL_EFFECTS.find(e=>e.id===sk)||TRAIL_EFFECTS[0],ps=parts(lo),run=0,leg=0,arm=0,hy=23,shoe=0;ctx.save();ctx.translate(p.x,p.y);ctx.shadowColor=ef.glow;ctx.shadowBlur=12;if(ef.wing){ctx.fillStyle="rgba(255,255,255,.6)";ctx.beginPath();ctx.ellipse(-5,36,18,34,-.55,0,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(15,23,42,.72)";ctx.beginPath();ctx.ellipse(52,37,18,34,.55,0,Math.PI*2);ctx.fill()}const drawLeg=(x,w,h)=>{ctx.save();ctx.translate(x,p.h-32);rr(-w/2,0,w,h,7);ctx.fill();ctx.restore()};ctx.fillStyle=ps.pants.main;if(ps.pants.style==="baggy"){const gr=ctx.createLinearGradient(10,p.h-32,38,p.h);gr.addColorStop(0,ps.pants.main);gr.addColorStop(1,ps.pants.detail);ctx.fillStyle=gr;drawLeg(18,13,31);drawLeg(32,13,31)}else if(ps.pants.style==="cargo"){drawLeg(18,12,31);drawLeg(32,12,31);ctx.fillStyle=ps.pants.detail;ctx.fillRect(11,p.h-20,6,5);ctx.fillRect(31,p.h-20,6,5)}else{drawLeg(18,11,28);drawLeg(32,11,28);ctx.fillStyle=ps.pants.detail;ctx.fillRect(13,p.h-5,9,2);ctx.fillRect(27,p.h-5,9,2)}ctx.fillStyle=ps.shoes.main;if(ps.shoes.style==="highTop"){rr(6,p.h-12,18,13,5);ctx.fill();rr(25,p.h-12,18,13,5);ctx.fill()}else{rr(6,p.h-8,20,9,5);ctx.fill();rr(24,p.h-8,22,9,5);ctx.fill()}if(ps.shoes.style==="wingShoes"){ctx.fillStyle="rgba(255,255,255,.72)";ctx.beginPath();ctx.ellipse(4,p.h-7,7,3.5,-.2,0,Math.PI*2);ctx.ellipse(47,p.h-7,7,3.5,.2,0,Math.PI*2);ctx.fill()}ctx.fillStyle=ps.shoes.sole;ctx.fillRect(8,p.h-2,18,2);ctx.fillRect(26,p.h-2,18,2);if(ps.top.style==="plainTee"){ctx.fillStyle=ps.top.main;rr(8,hy+3,32,26,9);ctx.fill();ctx.fillStyle=ps.top.accent;ctx.fillRect(13,hy+10,22,3)}else if(ps.top.style==="hoodie"){const gr=ctx.createLinearGradient(0,hy,48,hy+34);gr.addColorStop(0,ps.top.accent);gr.addColorStop(.18,ps.top.main);gr.addColorStop(1,"#09090b");ctx.fillStyle=gr;rr(5,hy,38,30,13);ctx.fill();ctx.fillStyle=ps.top.detail;rr(9,hy+17,30,13,7);ctx.fill()}else if(ps.top.style==="varsity"){ctx.fillStyle=ps.top.main;rr(8,hy,32,29,10);ctx.fill();ctx.fillStyle=ps.top.sleeve;rr(4,hy+2,9,24,6);ctx.fill();rr(35,hy+2,9,24,6);ctx.fill();ctx.fillStyle=ps.top.accent;ctx.fillRect(23,hy+2,3,25)}else if(ps.top.style==="layeredTee"){ctx.fillStyle=ps.top.inner;rr(7,hy+4,36,28,10);ctx.fill();ctx.fillStyle=ps.top.main;rr(9,hy,32,23,9);ctx.fill();ctx.fillStyle=ps.top.accent;ctx.fillRect(14,hy+8,22,3)}else{ctx.fillStyle=ps.top.main;rr(6,hy,37,32,12);ctx.fill();ctx.strokeStyle=ps.top.detail;for(let i=0;i<4;i++){ctx.beginPath();ctx.moveTo(9,hy+6+i*6);ctx.lineTo(40,hy+6+i*6);ctx.stroke()}ctx.fillStyle=ps.top.accent;ctx.fillRect(23,hy+3,2,23)}ctx.strokeStyle=ps.top.style==="varsity"?ps.top.sleeve:ps.top.main;ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(8,hy+15);ctx.lineTo(3,hy+31);ctx.moveTo(42,hy+15);ctx.lineTo(47,hy+31);ctx.stroke();
  ...
  const sg=ctx.createRadialGradient(19,10,3,24,15,17);sg.addColorStop(0,"#ffd6b4");sg.addColorStop(.55,ps.hair.skinTone||"#d6a37c");sg.addColorStop(1,"#9a6a4f");ctx.fillStyle=sg;ctx.beginPath();ctx.arc(24,15,15.5,0,Math.PI*2);ctx.fill();ctx.fillStyle=ps.hair.hair;if(ps.hair.style==="capFade"){ctx.beginPath();ctx.arc(22,10,17,Math.PI,Math.PI*2);ctx.fill();ctx.fillStyle=ps.hair.cap||"#ef4444";rr(9,-1,30,8,6);ctx.fill();ctx.fillRect(36,3,13,4)}else if(ps.hair.style==="fluffy"){ctx.beginPath();ctx.arc(15,9,8,0,Math.PI*2);ctx.arc(23,5,10,0,Math.PI*2);ctx.arc(31,8,8,0,Math.PI*2);ctx.arc(37,12,6,0,Math.PI*2);ctx.fill()}else if(ps.hair.style==="spike"){ctx.beginPath();ctx.moveTo(9,12);ctx.lineTo(13,2);ctx.lineTo(18,9);ctx.lineTo(23,0);ctx.lineTo(28,9);ctx.lineTo(34,2);ctx.lineTo(39,13);ctx.lineTo(39,16);ctx.lineTo(9,16);ctx.closePath();ctx.fill()}else if(ps.hair.style==="basicShort"){ctx.beginPath();ctx.arc(24,9,15,Math.PI,Math.PI*2);ctx.fill();ctx.fillRect(10,10,28,5)}else if(ps.hair.style==="longStreet"){ctx.beginPath();ctx.arc(24,9,16,Math.PI,Math.PI*2);ctx.fill();rr(9,8,8,28,7);ctx.fill();rr(31,8,8,28,7);ctx.fill()}else{ctx.beginPath();ctx.arc(18,8,10,Math.PI,Math.PI*2);ctx.arc(30,8,10,Math.PI,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(255,255,255,.08)";ctx.fillRect(23,1,2,12)}ctx.fillStyle="#0f172a";ctx.beginPath();ctx.arc(19,15,1.7,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(29,15,1.7,0,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(244,114,182,.38)";ctx.beginPath();ctx.arc(16,19,3,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(32,19,3,0,Math.PI*2);ctx.fill();ctx.restore()}
 ...
 useEffect(()=>{
  ...
  if(ps.top.style==="plainTee"){ctx.fillStyle=ps.top.main;rr(8,hy+3,32,26,9);ctx.fill();ctx.fillStyle=ps.top.accent;ctx.fillRect(13,hy+10,22,3)}else if(ps.top.style==="hoodie"){const gr=ctx.createLinearGradient(0,hy,48,hy+34);gr.addColorStop(0,ps.top.accent);gr.addColorStop(.18,ps.top.main);gr.addColorStop(1,"#09090b");ctx.fillStyle=gr;rr(5,hy,38,30,13);ctx.fill();ctx.fillStyle=ps.top.detail;rr(9,hy+17,30,13,7);ctx.fill()}
  ...
  else if(ps.hair.style==="basicShort"){ctx.beginPath();ctx.arc(24,9,15,Math.PI,Math.PI*2);ctx.fill();ctx.fillRect(10,10,28,5)}else if(ps.hair.style==="longStreet"){ctx.beginPath();ctx.arc(24,9,16,Math.PI,Math.PI*2);ctx.fill();rr(9,8,8,28,7);ctx.fill();rr(31,8,8,28,7);ctx.fill()}else{ctx.beginPath();ctx.arc(18,8,10,Math.PI,Math.PI*2);ctx.arc(30,8,10,Math.PI,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(255,255,255,.08)";ctx.fillRect(23,1,2,12)}
 ...
 const renderMiniPreview=()=> <div className="avatar-preview"><canvas ref={previewCanvasRef}/><div style={{position:"absolute",left:12,right:12,bottom:12,textAlign:"center"}}><div style={{fontWeight:1000,color:"#fde047"}}>{previewItem?.name||"Preview Outfit"}</div><div className="muted">{parts(previewLoadout).hair.name} • {parts(previewLoadout).top.name}<br/>{parts(previewLoadout).pants.name} • {parts(previewLoadout).shoes.name}<br/>{parts(previewLoadout).accessory.name} • {TRAIL_EFFECTS.find(e=>e.id===previewSkin)?.name||"No Trail"}</div></div></div>;
 ...
 const panel=()=> <><div className="card pad"><div className="tabs">{[{id:"leaderboard",label:"🏆 Rank"},{id:"friends",label:"👥 Friends"},{id:"daily",label:"🎯 Daily"},{id:"wardrobe",label:"🧢 Style"},{id:"settings",label:"⚙️ Login"}].map(t=><button key={t.id}onClick={()=>setActive(t.id)}className={`tab ${active===t.id?"active":""}`}>{t.label}</button>)}</div>
