...
<style>{`
.app-shell{min-height:100dvh;height:100dvh;padding-top:env(safe-area-inset-top);padding-bottom:env(safe-area-inset-bottom);overflow:hidden}.landing-shell,.shell-game{height:calc(100dvh - env(safe-area-inset-top) - env(safe-area-inset-bottom));display:flex;flex-direction:column}.landing-shell{display:grid;grid-template-columns:1.1fr .9fr;gap:12px;align-items:center;overflow:hidden}.landing-hero{max-height:100%;overflow:auto}.landing-hero .title{font-size:clamp(44px,7vw,84px);line-height:.9;margin:8px 0 10px}.landing-side{max-height:100%;overflow:auto}.landing-quick-nav{margin-top:10px}.landing-quick-nav small{display:block;color:#94a3b8;margin-bottom:6px}.landing-nav{padding-bottom:0;flex-wrap:wrap}.landing-nav button{font-size:12px;padding:6px 10px}.landing-features{grid-template-columns:repeat(2,minmax(0,1fr))}.topbar{display:flex;justify-content:space-between;gap:10px;align-items:center;padding:8px 0}.main-nav{display:flex;gap:8px;overflow:auto;padding-bottom:8px}.main-nav button{background:#111827;color:#e2e8f0;border:1px solid #334155;border-radius:999px;padding:8px 12px}.main-nav button.on{background:#facc15;color:#111827}.game-shell-main{flex:1;min-height:0;display:grid;grid-template-columns:1.2fr 1fr;gap:12px;overflow:hidden}.play-column{min-height:0;display:flex;flex-direction:column;gap:8px}.panel-column{min-height:0;overflow:auto}.canvaswrap{flex:1;min-height:220px}.canvaswrap canvas{max-height:100%;object-fit:contain}.panel{overflow:auto}@media (max-width:980px){.landing-shell,.game-shell-main{grid-template-columns:1fr}.panel-column{order:2}.play-column{order:1}.landing-side{display:none}}@media (max-width:720px){.topbar{flex-wrap:wrap}.shell-game{padding-bottom:72px}.panel-column{padding-bottom:10px}.main-nav{position:fixed;left:0;right:0;bottom:0;padding:8px max(8px,env(safe-area-inset-left)) calc(8px + env(safe-area-inset-bottom)) max(8px,env(safe-area-inset-right));background:#020617;border-top:1px solid #334155;z-index:20}.landing-shell{padding-bottom:12px}.landing-hero{overflow:auto}}
`}</style> </div>{profileModalView()}</>;

 if(screen==="landing")return <div className="app app-shell"><div className="shell landing-shell"><motion.div className="card hero landing-hero"initial={{opacity:0,y:18}}animate={{opacity:1,y:0}}><div className="pill">FRIENDS • SHOP PREVIEW • CLOUD SAVE</div><h1 className="title">SWAG<br/>NIGHT<br/><span>RUNNER</span></h1><p className="sub">วิ่งฝ่าเมืองกลางคืน เพิ่มเพื่อน แข่งกับเพื่อน แต่งตัวพร้อม Preview ไอเทม และ save ข้อมูลออนไลน์ • Username ใช้ A-Z/0-9 และ _ . - @ ได้</p><div className="formrow"><div className="input readonly-name">{user?`PLAYER: ${playerName}`:"LOGIN REQUIRED"}</div><Button onClick={reset}>{user?"▶ เข้าเล่น":"🔐 Login to Play"}</Button></div>{!user&&<div className="notice"style={{marginTop:10}}>🔐 Login to Play — บันทึกคะแนน โปรไฟล์ Avatar เพื่อน และของแต่งตัวออนไลน์</div>}<div className="grid4 landing-features"style={{marginTop:10}}><div className="notice">👥 Friends</div><div className="notice">🏆 Friends Leaderboard</div><div className="notice">🧥 Style Preview</div><div className="notice">☁️ Cloud Save</div></div></motion.div><motion.div className="card pad landing-side"initial={{opacity:0,y:18}}animate={{opacity:1,y:0}}><h3>{user?"Account Ready":"Quick Start"}</h3><p className="muted">{user?`Signed in as ${playerName}`:"Login แล้วเริ่มวิ่งได้ทันที"}</p><div className="grid2"><div className="stat"><small>Best</small><b>{bestScore}</b></div><div className="stat"><small>Wallet</small><b>🪙 {walletCoins}</b></div></div><Button style={{width:"100%",marginTop:10}} onClick={reset}>{user?"▶ Start Run":"🔐 Login to Play"}</Button><div className="landing-quick-nav"><small>Main Menu</small><div className="main-nav landing-nav">{[{id:"play",label:"Play"},{id:"shop",label:"Shop"},{id:"leaderboard",label:"Leaderboard"},{id:"friends",label:"Friends"},{id:"profile",label:"Profile"},{id:"settings",label:"Settings"},...(isDev?[{id:"dev",label:"Dev"}]:[])].map(n=><button key={n.id}className={appPanel===n.id?"on":""}onClick={()=>setAppPanel(n.id)}>{n.label}</button>)}</div></div><p className="muted" style={{marginTop:10}}>เลือกเมนูไว้ล่วงหน้า แล้วกด Start เพื่อเข้า panel ที่ต้องการ</p></motion.div></div></div>;
