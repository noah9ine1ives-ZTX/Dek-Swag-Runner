...
const OBSTACLE_TYPES={
 LOW_JUMP:{id:"LOW_JUMP",label:"Jump",icon:"↑",action:"jump",style:"#f97316",reward:35},
 HIGH_SLIDE:{id:"HIGH_SLIDE",label:"Slide",icon:"↓",action:"slide",style:"#38bdf8",reward:45},
 TALL_DOUBLE:{id:"TALL_DOUBLE",label:"Double",icon:"↑↑",action:"double",style:"#a78bfa",reward:70},
 MIXED:{id:"MIXED",label:"Mixed",icon:"↕",action:"mixed",style:"#facc15",reward:95}
};
...
function buildObstacle(type,x){const cfg=OBSTACLE_TYPES[type]||OBSTACLE_TYPES.LOW_JUMP;if(type==="HIGH_SLIDE")return{x,w:42,h:34,y:GROUND-108,type,requiredAction:cfg.action,label:cfg.label,icon:cfg.icon,color:cfg.style,reward:cfg.reward,passed:false,kind:"drone"};if(type==="TALL_DOUBLE")return{x,w:46,h:86,y:GROUND-86,type,requiredAction:cfg.action,label:cfg.label,icon:cfg.icon,color:cfg.style,reward:cfg.reward,passed:false,kind:"tall"};if(type==="MIXED")return{x,w:64,h:76,y:GROUND-76,type,requiredAction:cfg.action,label:cfg.label,icon:cfg.icon,color:cfg.style,reward:cfg.reward,passed:false,kind:"gate"};return{x,w:42,h:48,y:GROUND-48,type:"LOW_JUMP",requiredAction:"jump",label:"Jump",icon:"↑",color:OBSTACLE_TYPES.LOW_JUMP.style,reward:OBSTACLE_TYPES.LOW_JUMP.reward,passed:false,kind:"barrier"}}
function obstacleHitbox(o){if(o.type==="HIGH_SLIDE")return{x:o.x+4,y:o.y,w:o.w-8,h:o.h};if(o.type==="TALL_DOUBLE")return{x:o.x+4,y:o.y+2,w:o.w-8,h:o.h-2};if(o.type==="MIXED")return{x:o.x+5,y:o.y+8,w:o.w-10,h:o.h-8};return{x:o.x+4,y:o.y+4,w:o.w-8,h:o.h-4}}
...
g.trails.forEach(t=>{t.life--;t.x+=(t.vx||(-g.speed*.75*boost));t.y+=(t.vy||0);t.vy=(t.vy||0)+.03});g.trails=g.trails.filter(t=>t.life>0).slice(-42);g.particles.forEach(pt=>{pt.x+=pt.vx;pt.y+=pt.vy;pt.vy+=.25;pt.life--});g.particles=g.particles.filter(pt=>pt.life>0);g.texts.forEach(t=>{t.y-=.75;t.life--});g.texts=g.texts.filter(t=>t.life>0);g.build.forEach(b=>{b.x-=g.speed*.28*boost;if(b.x+b.w<0){b.x=W+Math.random()*120;b.w=70+Math.random()*45;b.h=90+Math.random()*190;b.win=Math.floor(4+Math.random()*7)}});g.stars.forEach(s=>{s.x-=g.speed*.08*boost;if(s.x<0)s.x=W});g.rain.forEach(r=>{r.x-=g.speed*.35*boost;r.y+=r.spd*boost;if(r.y>H+30||r.x<-30){r.x=Math.random()*W+40;r.y=-30}});const now=performance.now();if(now-ui.current>100||g.lives<=0){ui.current=now;setScore(g.score);setCoins(g.coins);setLives(g.lives);setCombo(Math.floor(g.combo));setMaxCombo(g.maxCombo);setStyleScore(g.style)}}
 ...
 if(o.x<W+80&&o.x>-80){ctx.font="900 18px system-ui";ctx.textAlign="center";ctx.fillStyle=o.color||"#fde047";ctx.fillText(o.icon||"↑",o.x+o.w/2,o.y-14);ctx.font="800 10px system-ui";ctx.fillStyle="rgba(226,232,240,.8)";ctx.fillText(o.label||"Jump",o.x+o.w/2,o.y-2)}
 ctx.restore();
