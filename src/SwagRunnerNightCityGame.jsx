...
function previewBodyType(type){if(locked)return lockToast();const nl=normLoadout({...previewLoadout,bodyType:type});setPreviewLoadout(nl);setPreviewItem({cat:"bodyType",id:type,name:type==="female"?"Female Runner":"Male Runner",price:0});setToast(`Preview ${type==="female"?"Female Runner":"Male Runner"}`)}
 ...
 function pressJump(){if(screen!=="game")return;if(status!=="playing")return reset();keys.current.Space=false;setTimeout(()=>{keys.current.Space=true;setTimeout(()=>keys.current.Space=false,120)},0)}
 ...
 useEffect(()=>{if(appPanel==="shop"&&active!=="wardrobe")setActive("wardrobe");if(appPanel==="leaderboard"&&active!=="leaderboard")setActive("leaderboard");if(appPanel==="friends"&&active!=="friends")setActive("friends");if((appPanel==="profile"||appPanel==="settings"||appPanel==="dev")&&active!=="settings")setActive("settings");},[appPanel]);
