...
<style>{`
.app-shell{min-height:100dvh;padding-top:env(safe-area-inset-top);padding-bottom:env(safe-area-inset-bottom);overflow:hidden}.shell-game{height:calc(100dvh - env(safe-area-inset-top) - env(safe-area-inset-bottom));display:flex;flex-direction:column}.topbar{display:flex;justify-content:space-between;gap:10px;align-items:center;padding:8px 0}.game-shell-main{flex:1;min-height:0;display:grid;grid-template-columns:1.2fr 1fr;gap:12px;overflow:hidden}.play-column{min-height:0;display:flex;flex-direction:column;gap:8px}.panel-column{min-height:0;overflow:auto}.canvaswrap{flex:1;min-height:220px}.canvaswrap canvas{max-height:100%;object-fit:contain}.panel{overflow:auto}@media (max-width:980px){.game-shell-main{grid-template-columns:1fr}.panel-column{order:2}.play-column{order:1}}@media (max-width:720px){.topbar{flex-wrap:wrap}.shell-game{padding-bottom:72px}.panel-column{padding-bottom:10px}}
`}</style> </div>{profileModalView()}</>;
 ...
 return <div className="app app-shell"><div className="shell shell-game"><header className="topbar"><div><h1 style={{margin:0,fontSize:"clamp(20px,3vw,34px)"}}>Swag Night Runner</h1><div className="muted">Player: <b>{playerName}</b> • {user?"Online":"Guest"}</div></div><div className="row"><span className="pill">{active}</span><span className="pill">🪙 {walletCoins}</span><Button onClick={reset}>{status==="playing"?"↻ Restart":user?"▶ Start":"🔐 Login"}</Button></div></header><main className="game-shell-main"><section className="play-column"><div className="stats">{[["Score",score],["Coins",`🪙 ${coins}`],["Lives",`❤️ ${lives}`],["Best",bestScore],["Combo",`x${combo}`],["Max",`x${maxCombo}`],["Style",styleScore]].map(([a,b])=><div className="stat"key={a}><small>{a}</small><b>{b}</b></div>)}</div><div className="canvaswrap"><canvas ref={canvasRef}width={W}height={H}onTouchStart={e=>{e.preventDefault();const t=e.changedTouches[0],r=e.currentTarget.getBoundingClientRect();if(t.clientX-r.left<r.width*.42)pressDuck(true);else pressJump()}}onTouchMove={e=>e.preventDefault()}onTouchEnd={e=>{e.preventDefault();pressDuck(false)}}/><div className="mobilehint"><span>แตะซ้ายค้าง = หมอบ</span><span>แตะขวา = กระโดด</span></div></div><div className="grid4"style={{marginTop:8}}><button className="btn dark"onTouchStart={e=>{e.preventDefault();pressDuck(true)}}onTouchEnd={e=>{e.preventDefault();pressDuck(false)}}onMouseDown={()=>pressDuck(true)}onMouseUp={()=>pressDuck(false)}>↓ หมอบ</button><button className="btn"onTouchStart={e=>{e.preventDefault();pressJump()}}onMouseDown={pressJump}>↑ กระโดด</button><button className="btn dark"disabled={locked}onClick={cycleQuality}>⚙️ Quality: {quality.toUpperCase()}</button><Button variant="outline"onClick={goLanding}disabled={status==="playing"}>← Landing</Button></div></section><section className="panel panel-column">{status==="gameover"&&<div className="card pad"><div className="muted">RESULT</div><div className="grade">Rank {grade(finalScore,maxCombo,styleScore)}</div><p>Score <b>{finalScore}</b> • Max Combo <b>x{maxCombo}</b></p></div>}{panel()}</section></main></div></div>
}
